"""Shared wire-protocol helpers for the Hakim LiveKit plugins.

Both `stt.py` and `tts.py` talk directly to Hakim's public REST/WebSocket
API (see https://tryhakim.ai/docs) over plain `websockets` /
`aiohttp` connections. This module
exists only to avoid duplicating the handful of things both plugins need:
region → host resolution, the `ws(s)://` upgrade URL, the auth header, and
a shared exception type for the server's `error` frame.
"""
...
def resolve_ws_url(path: str, *, region: Region = "auto", base_url: str | None = None) -> str:
    ...
    """
    if base_url:
        host = base_url.removeprefix("https://").removeprefix("http://").removeprefix("wss://").removeprefix("ws://")
        host = host.rstrip("/")
    else:
